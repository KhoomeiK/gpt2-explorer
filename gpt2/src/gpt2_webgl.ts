...
export type ForwardHooks = {
  onLayerEnd?: (layerIndex: number, resid: Float32Array, T: number, D: number) => void;
};
...
export class GPT2WebGL {
  ...
  public async forward(inputIds: number[], hooks?: ForwardHooks): Promise<Float32Array> {
    const gl = this.gl;
    ...
    for (let layer = 0; layer < this.nLayers; layer++) {
      ...
      // ---- Hook: emit per-layer residual stream (token-major, T*D floats) ----
      if (hooks?.onLayerEnd) {
        // GPU → CPU readback: row-major [H=L, W=D] which is already token-major contiguous
        // i.e., for token t in [0..L), its D features are at raw[t*D .. t*D+D)
        const resid = this._readTex(res2, this.nEmbeds, L).subarray(0, this.nEmbeds * L);
        hooks.onLayerEnd(layer, resid, L, this.nEmbeds);
      }
